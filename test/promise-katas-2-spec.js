...
describe('food', () => {
    it.only('returns the correct food', async () => {
        expect(await food()).to.equal("Cheese");
    })
});

describe('cat', () => {
    it.only('returns the correct ordered list of cats', async () => {
        expect(await cat()).to.deep.equal(["Bandit", "Berry", "Puss in boots", "Smokey"]);
    })
});

describe('dog', () => {
    it.only('returns the naughiest dog', async () => {
        expect(await dog()).to.deep.equal({name: "Mutley", naughty: 10});
    })
});

describe('joke', () => {
    it.only('returns a joke and answer, using Promise.all to fire of multiple requests at the same time.', async () => {
